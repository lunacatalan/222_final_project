...
```{r include = TRUE, message = FALSE, warning = FALSE}
ggplot(qazvin_clean, aes(x = mean_bn,
                 y = mean_bs)) +
  geom_point() +
  geom_smooth(method = "lm", # tell what to fit model to 
              formula = y ~ x, # association between variables
              se = FALSE) + #standard error
  labs(x = "Mean SAVI in Buffer",
       y = "Mean NDVI in Buffer") +
  theme_minimal()

# these are hella correlated
lm(mean_bs ~ mean_bn, data = qazvin_clean) %>% 
  summary()

```

My first analysis was looking at the difference between the NDVI and the SAVI values I calculated. We can be over 99% confident that the mean NDVI values and mean SAVI are correlated. 

```{r include = TRUE, message = FALSE, warning = FALSE}
# looking at gw level over time
ggplot(qazvin_clean) + 
  geom_point(aes(x = year, 
            y = gw_level)) +
  theme_bw()

# looking at precip over time 
ggplot(qazvin_clean) + 
  geom_point(aes(x = year, 
            y = annual_precip)) +
  theme_bw()
